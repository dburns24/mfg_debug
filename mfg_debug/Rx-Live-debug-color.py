...
import string
import visa
import lanconnectclass


SIGGEN_1 = lanconnectclass.Lanconnect('10.12.117.104')
SIGGEN_LO = lanconnectclass.Lanconnect('10.12.116.61')
SPECAN = lanconnectclass.Lanconnect('10.12.118.44')
...
with open('dataRxGain2.csv', 'w') as csv_file:
            ...
            for IF_FREQ in IF_FREQ_RANGES:                             
                ...
                for LO_AMPTD in LO_AMPTD_RANGES:
                    ...
                    for RF_FREQ in RF_FREQ_RANGES:
                        ...
                        if (h==2*len(RF_FREQ_RANGES)):
                            os.remove('dataRxGain2.csv')
                            h = 0
                            print("========================================================== dataRxGain1.csv file removed")
                            fieldnames = ["RF_Freq", "Gain"]
                            with open('dataRxGain2.csv', 'w') as csv_file:
                        ...
                        for RF_INPUT in RF_INPUT_RANGES:
                            ...
                            SPECAN.query('*OPC?') 
                            markerX = SPECAN.query('CALC1:MARK1:X?')
                        ...
                        if (h>1*len(RF_FREQ_RANGES)):                             
                            with open('dataRxGain2.csv', 'a') as csv_file:
                                csv_writer = csv.DictWriter(csv_file, fieldnames=fieldnames2)
                                if RF_FREQ == 44.0:
                                    info = {"RF_Freq": RF_FREQ, "Gain": float("nan")}
                                    csv_writer.writerow(info)
